...
def parse_data(data):
  elves = {(x, y) for y, line in enumerate(data.splitlines()) for x, c in enumerate(line) if c == "#"}
  return elves
...
def propose(elf, elves, directions):
  x, y = elf
  neighbors = {(x + dx, y + dy) for dy in [-1, 0, 1] for dx in [-1, 0, 1] if not (dx == 0 and dy == 0)}
