...
def make_test_function(string):
  return lambda item: item % int(string) == 0
...
def get_monkeys(data):
  ...
  for block in data.split("\n\n"):
    ...
    monkey = Monkey(
      monkeys, int(pk), starting_items, operation, int(divisible_by), int(a), int(b)
    )
...
def play_round(monkeys, limit_worry_level):
  for monkey in monkeys:
    monkey.take_turn(limit_worry_level)

def play_game(monkeys, turns, limit_worry_level):
  for _ in range(turns):
    play_round(monkeys, limit_worry_level)
...
def solve(data):
  ...
  p1 = play_game(monkeys, 20, True)
  p2 = play_game(monkeys, 10000, False)
